...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Initial fetch
    axios.get(apiUrl('/api/dashboard')).then(res => setData(res.data));
  ...
  const toggleWeather = async () => {
    ...
    await axios.post(apiUrl('/api/weather/toggle'), { active: newState });
  };
  ...
  const handleRouteRequest = async () => {
    ...
    try {
      ...
      const res = await axios.post(apiUrl('/api/route'), {
  ...
  const handleSegmentClick = async (seg) => {
    ...
    try {
      const res = await axios.get(apiUrl(`/api/predict/${seg.id}`));
