...
export default function FieldReport({ isOnline }) {
  ...
  useEffect(() => {
    ...
    if (isOnline) {
      axios.get(apiUrl('/api/dashboard')).then(res => setSegments(res.data.segments)).catch(() => {});
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isOnline) {
      try {
        await axios.post(apiUrl('/api/incidents'), payload);
  ...
  const handleSync = async () => {
    ...
    for (const item of newQueue) {
      try {
        await axios.post(apiUrl('/api/incidents'), item);
      } catch (e) {
        failed.push(item);
      }
    }
  ...
  const capturePhoto = async () => {
    if (videoRef.current && canvasRef.current) {
      ...
      try {
        const res = await axios.post(apiUrl('/api/vision/analyze'), {
          photo_url: dataUrl,
  ...
  const simulateMeshRelay = async (item) => {
    ...
    try {
      await axios.post(apiUrl('/api/incidents'), { ...item, relayed_via_mesh: true });
